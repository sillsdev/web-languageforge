...
def build_ddp_data(tree, guids_by_abbr, lang):
    ...
    for option in tree.iter('option'):
        ...
        record['value'] = option.find("./key").text
...
def build_question_data(tree, lang):
    ...
    for domain in tree.iter('semantic-domain'):
        # take the number before the first space as the key
        id = domain.get('id')
        key = id.split()[0]
...
def main():
    ...
    for lang in ALL_LANGUAGES:  # Replace with LANGUAGES_TO_PROCESS if desired
        ...
        write_json(data, lang, fname, OUTPUT_PREFIX_DOMAINS.format(__file__, underscore_lang), OUTPUT_SUFFIX)
        ...
        write_json(data, lang, fname, OUTPUT_PREFIX_QUESTIONS.format(__file__, underscore_lang), OUTPUT_SUFFIX)
