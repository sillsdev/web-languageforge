...
gulp.task('test-local', function (cb) {
  execute('php test/php/languageforge/lexicon/LiftImportFlex_Test.php', function () {
    cb(null);
  });
});

gulp.task('test-local-watch', function () {
  gulp.watch(['./src/Api/Model/**/*.php', './src/Api/Library/**/*.php',
    './test/php/languageforge/lexicon/**/*.php', '!./src/vendor/'], ['test-local']);
});
...
gulp.task('copy-prod-db', ['backup-prod-db', 'cleanup-backup-prod-db', 'copy-backup-to-local',
  'restore-local-db'], function (cb) {
    cb(null);
  }
);
