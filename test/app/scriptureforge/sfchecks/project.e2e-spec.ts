  ...
  */
  ...
  describe('project manager', () => {
    ...
    it('can create a new text (input text area)', () => {
      ...
      expect<any>(SfProjectPage.textLink(sampleTitle).isDisplayed()).toBe(true);
    });

    it('can click through to newly created text', () => {
      SfProjectPage.textLink(sampleTitle).click();
      browser.navigate().back();
    });
    ...
    it('can archive the text that was just created', () => {
      ...
      expect<any>(archiveButton.isEnabled()).toBe(false);
      expect<any>(SfProjectPage.textLink(sampleTitle).isPresent()).toBe(false);
    });
    ...
    it('can use the chapter trimmer to trim the USX when creating a new text', () => {
      ...
      Utils.sendText(projectPage.newText.usx, projectPage.testData.longUsx1);
      ...
      projectPage.newText.saveButton.click();
      expect<any>(SfProjectPage.textLink(newTextTitle).isDisplayed()).toBe(true);
      SfProjectPage.textLink(newTextTitle).click();
