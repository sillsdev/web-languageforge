...
var SfQuestionPage = function() {
  ...
  this.answers.add = function(answer) {
    this.answerCtrl = browser.element(by.id('comments')); // Using ID "Comments" contains Answers and Comments
  ...
  this.answers.edit = function(answer) {
    this.editCtrl     = page.answers.last().$('.answer').element(by.linkText('edit'));
  ...
  this.answers.archive = function(index) {
    if (index === "") {
      page.answers.last().$('.answer').element(by.linkText('delete')).click();
    } else {
      //console.log('should delete answer at index ' + index);
      page.answers.list.get(index).$('.answer').element(by.linkText('delete')).click();
    }
  ...
  this.answers.flags.lastButtonSet = function () {
    return page.answers.last().$('.answer').element(by.css('.icon-flag'));
  };
  this.answers.flags.lastButtonClear = function () {
    return page.answers.last().$('.answer').element(by.css('.icon-flag-alt'));    
  };
  ...
  this.comments.edit = function(comment) {
    this.editCtrl     = page.comments.last().element(by.linkText('edit'));

    this.editCtrl.click();

    // Clicking 'edit' changes the DOM so these handles are updated here
    var commentsField = page.comments.last().$('textarea');
    var saveCtrl      = page.comments.last().element(by.partialButtonText('Save'));
  ...
  this.comments.archive = function(index) {
    if (index === "") {
      page.comments.last().element(by.linkText('delete')).click();
    } else {
      //console.log('should delete comment at index ' + index);
      page.comments.list.get(index).element(by.linkText('delete')).click();
    };
    util.clickModalButton('Delete');
  };
