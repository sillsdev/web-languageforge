...
class TestUsxTrimHelper extends UnitTestCase {
	...
	function hasChapter($xml, $chNum, $errMsg = "") {
		$result = $xml->xpath("//chapter[@number=$chNum]");
		return (!empty($result));
	}

	function hasVerse($xml, $chNum, $vsNum) {
		$nextCh = $chNum+1;
	...
	function testTrim_acrossChapterBoundary() {
		$usx = MongoTestEnvironment::usxSample();
	...
	function testTrim_zeroEndVerse() {
	...
	function testTrim_zeroEndChapter() {
	...
	function testTrim_zeroStartVerse() {
	...
	function testTrim_zeroStartChapter() {
