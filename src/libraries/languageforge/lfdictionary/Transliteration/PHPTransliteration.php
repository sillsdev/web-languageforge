...
class PHPTransliteration implements TransliterationInterface {
	...
	protected function replace($code, $langcode, $unknown_character) {
		LoggerFactory::getLogger()->logDebugMessage("Langcode: $langcode / CODE: $code");
		if ($code < 0x80) {
			// Already lower ASCII.
			return chr($code);
		}
		...
		LoggerFactory::getLogger()->logDebugMessage("BANK: $bank / CODE: $code / LANG: $langcode");
		$code = $code & 0xff;
	...
	protected function readLanguageOverrides($langcode) {
		...
		if (is_file($file)) {
			LoggerFactory::getLogger()->logDebugMessage("readLanguageOverrides: " . $file);
			include_once $file;
		}
	...
	protected function readGenericData($bank) {
		...
		if (is_file($file)) {
			LoggerFactory::getLogger()->logDebugMessage("readGenericData: " . $file);
			include $file;
		}
