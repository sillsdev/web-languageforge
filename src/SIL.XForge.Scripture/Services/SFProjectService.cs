...
namespace SIL.XForge.Scripture.Services
{
    public class SFProjectService : ProjectService<SFProjectResource, SFProjectEntity>
    {
            ...
            IDictionary<string, string> relationships)
        {
            ...
            {
                // if currently running sync job for project is found, delete it
                string runningJobId = await _jobs.Query()
                    .Where(j => j.ProjectRef == id &&
                        (j.State == SyncJobEntity.PendingState || j.State == SyncJobEntity.SyncingState))
                    .Select(j => j.Id).SingleOrDefaultAsync();
                if (runningJobId != null)
                {
                    SyncJobEntity runningJob = await _jobs.DeleteAsync(runningJobId);
                    if (runningJob != null)
                    {
                        if (SyncJobEntity.ActiveStates.Contains(runningJob.State))
                            _backgroundJobClient.Delete(runningJob.BackgroundJobId);
                    }
                }
