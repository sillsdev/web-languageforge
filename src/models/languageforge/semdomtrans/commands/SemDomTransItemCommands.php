...
class SemDomTransItemCommands
{
	public static function update($data, $projectId) {
		...
		if ($previousItemModel->name->translation != $data["name"]["translation"] && 
		...
		if ($previousItemModel->description->translation != $data["description"]["translation"] &&
		...
		for ($i = 0; $i < count($s->questions); $i++ ) {
			...
			if ($previousItemModel->questions[$i]->question->translation != $data["questions"][$i]["question"]["translation"]
			...
			if ($previousItemModel->questions[$i]->terms->translation != $data["questions"][$i]["terms"]["translation"]
		...
		for ($i = 0; $i < count($s->searchKeys); $i++ ) {
			...
			if ($previousItemModel->searchKeys[$i]->translation != $data["searchKeys"][$i]["translation"]
