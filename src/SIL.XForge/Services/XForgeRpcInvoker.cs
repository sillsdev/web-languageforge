...
namespace SIL.XForge.Services
{
    ...
    public class XForgeRpcInvoker : IRpcInvoker
    {
        ...
        private ConcurrentDictionary<Type, ObjectFactory> objectFactoryCache { get; }
            = new ConcurrentDictionary<Type, ObjectFactory>();
        private ConcurrentDictionary<Type, (List<IAuthorizeData>, bool)> classAttributeCache { get; }
            = new ConcurrentDictionary<Type, (List<IAuthorizeData>, bool)>();
        private ConcurrentDictionary<MethodInfo, (List<IAuthorizeData>, bool)> methodAttributeCache { get; }
            = new ConcurrentDictionary<MethodInfo, (List<IAuthorizeData>, bool)>();
        ...
        public async Task<List<RpcResponse>> InvokeBatchRequestAsync(IList<RpcRequest> requests, RpcPath path,
        ...
        public async Task<RpcResponse> InvokeRequestAsync(RpcRequest request, RpcPath path, IRouteContext routeContext)
        {
            ...
            try
            {
                (RpcMethodInfo rpcMethod, Type invokeType) = this.GetMatchingMethod(path, request,
                    routeContext.RouteProvider, routeContext.RequestServices);
                ...
                if (isAuthorized)
                {
                    ...
                    object result = await this.InvokeAsync(rpcMethod, invokeType, path, routeContext.RequestServices);
        ...
        {
            (List<IAuthorizeData> authorizeDataListClass, bool allowAnonymousOnClass) = this.classAttributeCache
                .GetOrAdd(methodInfo.DeclaringType, GetClassAttributeInfo);
            (List<IAuthorizeData> authorizeDataListMethod, bool allowAnonymousOnMethod) = this.methodAttributeCache
                .GetOrAdd(methodInfo, GetMethodAttributeInfo);
                ...
                else
                {
                    ...
                    if (authResult.Succeeded)
                    {
                        this.logger?.LogDebug(
                            $"Authorization was successful for user '{routeContext.User.Identity.Name}'.");
                    }
                    else
                    {
                        this.logger?.LogInformation(
                            $"Authorization failed for user '{routeContext.User.Identity.Name}'.");
                        return false;
                    }
                }
            }
        ...
        private (RpcMethodInfo, Type) GetMatchingMethod(RpcPath path, RpcRequest request,
            IRpcRouteProvider routeProvider, IServiceProvider serviceProvider)
        {
            if (request == null)
            {
                throw new ArgumentNullException(nameof(request));
            ...
            Dictionary<MethodInfo, Type> allMethods = this.GetRpcMethods(path, routeProvider);

            List<RpcMethodInfo> matches = this.rpcRequestMatcher.FilterAndBuildMethodInfoByRequest(
                allMethods.Keys.ToList(), request);
            ...
            if (matches.Count > 1)
            {
                ...
                string errorMessage = "More than one method matched the rpc request. Unable to invoke due to ambiguity."
                    + " Methods that matched the same name: " + string.Join(", ", methodInfoList);
            ...
            {
                //Log diagnostics
                string methodsString = string.Join(", ", allMethods.Select(m => m.Key.Name));
            ...
            return (rpcMethod, allMethods[rpcMethod.Method]);
        }
        ...
        private Dictionary<MethodInfo, Type> GetRpcMethods(RpcPath path, IRpcRouteProvider routeProvider)
        {
            var methods = new Dictionary<MethodInfo, Type>();
            foreach (ControllerPublicMethodProvider methodProvider in routeProvider.GetMethodsByPath(path))
            {
                foreach (MethodInfo methodInfo in methodProvider.GetRouteMethods())
                {
                    methods.Add(methodInfo, methodProvider.Types[0]);
                }
            }
            return methods;
        }
        ...
        /// </summary>
        /// <exception cref="RpcInvalidParametersException">Thrown when conversion of parameters fails or when invoking
        /// the method is not compatible with the parameters</exception>
        /// <param name="parameters">List of parameters to invoke the method with</param>
        /// <returns>The result of the invoked method</returns>
        private async Task<object> InvokeAsync(RpcMethodInfo methodInfo, Type invokeType, RpcPath path,
            IServiceProvider serviceProvider)
        {
            ...
            if (serviceProvider != null)
            {
                ...
                ObjectFactory objectFactory = this.objectFactoryCache.GetOrAdd(invokeType,
                    (t) => ActivatorUtilities.CreateFactory(t, new Type[0]));
            ...
            {
                throw new RpcException(RpcErrorCode.InternalError, "Exception occurred from target method execution.",
                    ex);
            }
            catch (Exception ex)
            {
                throw new RpcException(RpcErrorCode.InvalidParams,
                    "Exception from attempting to invoke method. Possibly invalid parameters for method.", ex);
            }
        }
        ...
        /// <returns>Awaits a Task and returns its result if object is a Task, otherwise returns the same object given
        /// </returns>
