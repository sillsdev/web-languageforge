...
export class UserManagementAppController implements angular.IController {
  ...
  $onInit(): void {
    ...
    this.sessionService.getSession().then((session) => {
      this.rights.remove = session.hasProjectRight(this.sessionService.domain.USERS, this.sessionService.operation.DELETE);
      this.rights.add = session.hasProjectRight(this.sessionService.domain.USERS, this.sessionService.operation.CREATE);
      this.rights.changeRole = session.hasProjectRight(this.sessionService.domain.USERS, this.sessionService.operation.EDIT);
