...
angular.module('registration',
  ['bellows.services', 'ui.bootstrap', 'palaso.ui.notice', 'palaso.ui.utils'])
  .controller('UserCtrl', ['$scope', '$location', 'userService', 'sessionService',
  ...
  function ($scope, $location, userService, sessionService,
    ...
    $scope.registerUser = function (record) {
      ...
      userService.updateFromRegistration(validationKey, record, function (result) {
        ...
        if (result.ok) {
          if (result.data) {
            notice.push(notice.SUCCESS, 'Thank you, ' + record.name +
              ', for joining!  You can now login.');
