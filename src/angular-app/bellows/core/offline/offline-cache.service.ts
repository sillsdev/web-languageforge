...
export class OfflineCacheService {
  ...
  private addProjectId(object: any, projectId: string) {
  ...
  private removeProjectId(object: any) {
  ...
  getAllFromStore = (storeName: string, projectId?: string): angular.IPromise<any> => {
    ...
    return this.$q.when(this.getStore(storeName).iterate((value, key, iterationNumber) => {
      if(value['projectId'] === projectId) results.push(this.removeProjectId(value));
    }).then(() => {
      return results;
    }));
  }
  ...
  getOneFromStore = (storeName: string, key: string): angular.IPromise<any> => {
    return this.$q.when(this.getStore(storeName).getItem(key).then(item => {
      return this.removeProjectId(item);
    }));
  }
  ...
  setObjectsInStore = (storeName: string, projectId: string, items: any[],
    ...
    return this.$q.all(items.map(item => {
      return store.setItem(item.id, this.addProjectId(item, projectId));
    }));
  }
