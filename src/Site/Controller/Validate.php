...
class Validate extends Base
{
    ...
    public function checkAndRedirect(Application $app, $validateKey = "")
    {
        if (self::check($app, $validateKey)) {
            $app["session"]
                ->getFlashBag()
                ->add("infoMessage", 'Congratulations!  Your email has been validated and you\'re ready to login.');
        }
    ...
    public function processInviteAndRedirect(Application $app, $inviteToken = "")
    {
        ...
        try {
            ...
            $errorString =
                "This invite link is not valid, it may have been disabled. Please check with the project manager";
        ...
        if ($this->isLoggedIn($app)) {
            ...
            $redirectPath =
                ...
                base64_encode("Please log in or create an account to access this project");
            return $app->redirect($redirectPath);
        }
    }
}
