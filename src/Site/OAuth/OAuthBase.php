...
abstract class OAuthBase extends Base
{
    ...
    public function oauthCallback(Request $request, Application $app)
    {
        ...
        if (
            $app["session"]->has(OAuthBase::SESSION_KEY_OAUTH_ACCESS_TOKEN) &&
                ...
                if (!$userModel->id->asString()) {
                    ...
                    if (!$success) {
                        $this->addErrorMessage(
                            $app,
                            'Sorry, we couldn\'t process the ' .
                                ucwords($this->getProviderName()) .
                                " login data. This may be a temporary failure, so please try again. If the problem persists, try logging in with a username and password instead."
                        );
                    }
     ...
     * @param string $email
     */
    protected function setOAuthDetailsInSession(
        Application $app,
