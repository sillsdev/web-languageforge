...
class LexDbeDto
{
    ...
    public static function encode($projectId, $userId, $lastFetchTime = null, $offset = 0)
    {
        ...
        if ($lastFetchTime) {
            ...
            $data["itemTotalCount"] =
                $entriesModel->totalCount > $commentsModel->totalCount
                    ? $entriesModel->totalCount
                    : $commentsModel->totalCount;
            $data["itemCount"] =
                $entriesModel->count > $commentsModel->count ? $entriesModel->count : $commentsModel->count;
            $data["offset"] = $offset;
        }
